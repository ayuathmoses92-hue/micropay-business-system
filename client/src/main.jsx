...
function Customers({user}){const [rows,setRows]=useState([]);const [open,setOpen]=useState(false);const [editing,setEditing]=useState(null);const [f,setF]=useState({});const load=()=>api("/customers").then(setRows).catch(e=>alert(e.message));useEffect(()=>{load()},[]);const save=async e=>{e.preventDefault();try{await api(editing?`/customers/${editing.id}`:"/customers",{method:editing?"PATCH":"POST",body:JSON.stringify(f)});setOpen(false);setEditing(null);setF({});load()}catch(err){alert(err.message)}};const edit=r=>{setEditing(r);setF({name:r.name||"",contact_person:r.contact_person||"",phone:r.phone||"",email:r.email||"",address:r.address||"",tax_number:r.tax_number||"",notes:r.notes||""});setOpen(true)};const remove=async r=>{if(!window.confirm(`Delete customer ${r.name}? This can only be done if the customer has no quotations, invoices, or credit notes.`))return;try{await api(`/customers/${r.id}`,{method:"DELETE"});load()}catch(err){alert(err.message)}};return <><div className="toolbar"><h1>Customers</h1><button onClick={()=>{setEditing(null);setF({});setOpen(true)}}>+ New Customer</button></div>{open&&<div className="panel"><h2>{editing?`Edit ${editing.customer_code}`:"New Customer"}</h2><form onSubmit={save}><div className="form-grid">{["name","contact_person","phone","email","address","tax_number","notes"].map(k=><div key={k}><label>{k.replaceAll("_"," ")}{k==="name"?" *":""}</label>{k==="notes"||k==="address"?<textarea value={f[k]||""} onChange={e=>setF({...f,[k]:e.target.value})}/>:<input type={k==="email"?"email":"text"} required={k==="name"} value={f[k]||""} onChange={e=>setF({...f,[k]:e.target.value})}/>}</div>)}</div><button>{editing?"Save Changes":"Save Customer"}</button><button type="button" onClick={()=>{setOpen(false);setEditing(null);setF({})}}>Cancel</button></form></div>}<Table cols={["customer_code","name","contact_person","phone","email"]} rows={rows} actions={r=><><button type="button" onClick={()=>edit(r)}>Edit</button>{user?.role==="ADMIN"&&<button type="button" onClick={()=>remove(r)}>Delete</button>}</>}/></>}
