...
function InventoryModule({user}){
  ...
  const movementDateField=tab==="Transfers"?"transfer_date":tab==="Receipts"?"receipt_date":tab==="Issues"?"issue_date":"adjustment_date";
  const startNewMovement=()=>{const next={...f};next[movementDateField]=new Date().toISOString().slice(0,10);setF(next);setLines([{item_id:"",quantity:"",unit_cost:"0"}]);setFormOpen(true)};
  return <><div className="toolbar"><div><h1>Inventory & Warehousing</h1><p className="muted">Items, warehouses, stock movements and inventory control.</p></div>{tab==="Items"||tab==="Warehouses"||tab==="Setup"?<button onClick={()=>{setEditing(null);setF({});setFormOpen(true)}}>+ New {tab==="Items"?"Item":tab==="Warehouses"?"Warehouse":"Category"}</button>:tab!=="Stock"&&<button onClick={startNewMovement}>+ New {tab.slice(0,-1)}</button>}</div>
...
function HRDashboard(){
  const [d,setD]=useState(null);useEffect(()=>{api("/hr/dashboard").then(setD).catch(e=>alert(e.message))},[]);if(!d)return <p>Loading HR dashboard...</p>;
  return <><div className="toolbar"><div><h1>Human Resources</h1><p className="muted">Workforce overview and HR administration.</p></div></div><div className="cards"><div className="card"><small>TOTAL EMPLOYEES</small><strong>{d.employees.total}</strong></div><div className="card"><small>ACTIVE</small><strong>{d.employees.active}</strong></div><div className="card"><small>PROBATION</small><strong>{d.employees.probation}</strong></div><div className="card"><small>ON LEAVE</small><strong>{d.employees.on_leave}</strong></div><div className="card"><small>ACTIVE CONTRACTS</small><strong>{d.contracts.active}</strong></div><div className="card"><small>EXPIRING IN 60 DAYS</small><strong>{d.contracts.expiring_60_days}</strong></div></div><div className="panel"><div className="section-title"><h2>Employees by Department</h2></div><Table cols={["name","employee_count"]} rows={d.departments}/></div></>;
}
function HRDepartments(){
  const [rows,setRows]=useState([]),[open,setOpen]=useState(false),[editing,setEditing]=useState(null),[f,setF]=useState({});const load=()=>api("/hr/departments").then(setRows).catch(e=>alert(e.message));useEffect(()=>{load()},[]);const save=async e=>{e.preventDefault();try{await api(editing?`/hr/departments/${editing.id}`:"/hr/departments",{method:editing?"PATCH":"POST",body:JSON.stringify(f)});setOpen(false);setEditing(null);setF({});load()}catch(e){alert(e.message)}};return <><div className="toolbar"><div><h1>Departments</h1><p className="muted">Maintain the HR organizational structure.</p></div><button onClick={()=>{setEditing(null);setF({});setOpen(true)}}>+ New Department</button></div>{open&&<div className="panel hr-form-panel"><h2>{editing?"Edit Department":"New Department"}</h2><form className="hr-form" onSubmit={save}><div className="form-grid"><div><label>Code *</label><input required value={f.code||""} onChange={e=>setF({...f,code:e.target.value})}/></div><div><label>Name *</label><input required value={f.name||""} onChange={e=>setF({...f,name:e.target.value})}/></div><div className="form-field-full"><label>Description</label><textarea value={f.description||""} onChange={e=>setF({...f,description:e.target.value})}/></div></div><div className="form-actions"><button>Save</button><button type="button" onClick={()=>setOpen(false)}>Cancel</button></div></form></div>}<Table cols={["code","name","employee_count","active"]} rows={rows} actions={r=><button onClick={()=>{setEditing(r);setF(r);setOpen(true)}}>Edit</button>}/></>;
}
function HRPositions(){
 const [rows,setRows]=useState([]),[deps,setDeps]=useState([]),[open,setOpen]=useState(false),[editing,setEditing]=useState(null),[f,setF]=useState({});const load=async()=>{try{const [r,d]=await Promise.all([api("/hr/positions"),api("/hr/departments")]);setRows(r);setDeps(d)}catch(e){alert(e.message)}};useEffect(()=>{load()},[]);const save=async e=>{e.preventDefault();try{await api(editing?`/hr/positions/${editing.id}`:"/hr/positions",{method:editing?"PATCH":"POST",body:JSON.stringify(f)});setOpen(false);setEditing(null);setF({});load()}catch(e){alert(e.message)}};return <><div className="toolbar"><div><h1>Positions</h1><p className="muted">Job titles and departmental assignments.</p></div><button onClick={()=>{setEditing(null);setF({});setOpen(true)}}>+ New Position</button></div>{open&&<div className="panel hr-form-panel"><form className="hr-form" onSubmit={save}><div className="form-grid"><div><label>Code *</label><input required value={f.code||""} onChange={e=>setF({...f,code:e.target.value})}/></div><div><label>Title *</label><input required value={f.title||""} onChange={e=>setF({...f,title:e.target.value})}/></div><div><label>Department</label><select value={f.department_id||""} onChange={e=>setF({...f,department_id:e.target.value})}><option value="">Select</option>{deps.filter(x=>x.active).map(x=><option key={x.id} value={x.id}>{x.name}</option>)}</select></div><div className="form-field-full"><label>Description</label><textarea value={f.description||""} onChange={e=>setF({...f,description:e.target.value})}/></div></div><div className="form-actions"><button>Save</button><button type="button" onClick={()=>setOpen(false)}>Cancel</button></div></form></div>}<Table cols={["code","title","department_name","active"]} rows={rows} actions={r=><button onClick={()=>{setEditing(r);setF(r);setOpen(true)}}>Edit</button>}/></>;
}
function HREmployees(){
 const blank={employment_type:"FULL_TIME",employment_status:"ACTIVE",hire_date:new Date().toISOString().slice(0,10)};const [rows,setRows]=useState([]),[deps,setDeps]=useState([]),[pos,setPos]=useState([]),[open,setOpen]=useState(false),[editing,setEditing]=useState(null),[f,setF]=useState(blank);const load=async()=>{try{const [r,d,p]=await Promise.all([api("/hr/employees"),api("/hr/departments"),api("/hr/positions")]);setRows(r);setDeps(d);setPos(p)}catch(e){alert(e.message)}};useEffect(()=>{load()},[]);const save=async e=>{e.preventDefault();try{await api(editing?`/hr/employees/${editing.id}`:"/hr/employees",{method:editing?"PATCH":"POST",body:JSON.stringify(f)});setOpen(false);setEditing(null);setF(blank);load()}catch(e){alert(e.message)}};const edit=r=>{setEditing(r);setF({...r,hire_date:r.hire_date?.slice(0,10)||"",date_of_birth:r.date_of_birth?.slice(0,10)||"",separation_date:r.separation_date?.slice(0,10)||""});setOpen(true)};return <><div className="toolbar"><div><h1>Employees</h1><p className="muted">Employee master records and employment assignments.</p></div><button onClick={()=>{setEditing(null);setF(blank);setOpen(true)}}>+ New Employee</button></div>{open&&<div className="panel hr-form-panel"><h2>{editing?`Edit ${editing.employee_no}`:"New Employee"}</h2><form className="hr-form" onSubmit={save}><div className="form-grid"><div><label>Employee No *</label><input required value={f.employee_no||""} onChange={e=>setF({...f,employee_no:e.target.value})}/></div><div><label>First Name *</label><input required value={f.first_name||""} onChange={e=>setF({...f,first_name:e.target.value})}/></div><div><label>Middle Name</label><input value={f.middle_name||""} onChange={e=>setF({...f,middle_name:e.target.value})}/></div><div><label>Last Name *</label><input required value={f.last_name||""} onChange={e=>setF({...f,last_name:e.target.value})}/></div><div><label>Gender</label><select value={f.gender||""} onChange={e=>setF({...f,gender:e.target.value})}><option value="">Select</option><option>Male</option><option>Female</option><option>Other</option></select></div><div><label>Date of Birth</label><input type="date" value={f.date_of_birth||""} onChange={e=>setF({...f,date_of_birth:e.target.value})}/></div><div><label>Phone</label><input value={f.phone||""} onChange={e=>setF({...f,phone:e.target.value})}/></div><div><label>Email</label><input type="email" value={f.email||""} onChange={e=>setF({...f,email:e.target.value})}/></div><div><label>Department</label><select value={f.department_id||""} onChange={e=>setF({...f,department_id:e.target.value,position_id:""})}><option value="">Select</option>{deps.filter(x=>x.active).map(x=><option key={x.id} value={x.id}>{x.name}</option>)}</select></div><div><label>Position</label><select value={f.position_id||""} onChange={e=>setF({...f,position_id:e.target.value})}><option value="">Select</option>{pos.filter(x=>x.active&&(!f.department_id||x.department_id===f.department_id)).map(x=><option key={x.id} value={x.id}>{x.title}</option>)}</select></div><div><label>Employment Type</label><select value={f.employment_type||"FULL_TIME"} onChange={e=>setF({...f,employment_type:e.target.value})}>{["FULL_TIME","PART_TIME","CASUAL","CONTRACT","INTERN","CONSULTANT"].map(x=><option key={x}>{x.replaceAll("_"," ")}</option>)}</select></div><div><label>Status</label><select value={f.employment_status||"ACTIVE"} onChange={e=>setF({...f,employment_status:e.target.value})}>{["ACTIVE","PROBATION","ON_LEAVE","SUSPENDED","SEPARATED"].map(x=><option key={x}>{x.replaceAll("_"," ")}</option>)}</select></div><div><label>Hire Date *</label><input type="date" required value={f.hire_date||""} onChange={e=>setF({...f,hire_date:e.target.value})}/></div><div className="form-field-full"><label>Address</label><textarea value={f.address||""} onChange={e=>setF({...f,address:e.target.value})}/></div><div className="form-field-full"><label>Notes</label><textarea value={f.notes||""} onChange={e=>setF({...f,notes:e.target.value})}/></div></div><div className="form-actions"><button>{editing?"Save Changes":"Create Employee"}</button><button type="button" onClick={()=>setOpen(false)}>Cancel</button></div></form></div>}<Table cols={["employee_no","full_name","department_name","position_title","employment_type","employment_status"]} rows={rows} actions={r=><button onClick={()=>edit(r)}>Edit</button>}/></>;
}
function HRContracts(){
 const blank={contract_type:"PERMANENT",status:"ACTIVE",currency_code:"USD",start_date:new Date().toISOString().slice(0,10)};const [rows,setRows]=useState([]),[emps,setEmps]=useState([]),[open,setOpen]=useState(false),[editing,setEditing]=useState(null),[f,setF]=useState(blank);const load=async()=>{try{const [r,e]=await Promise.all([api("/hr/contracts"),api("/hr/employees")]);setRows(r);setEmps(e)}catch(e){alert(e.message)}};useEffect(()=>{load()},[]);const save=async e=>{e.preventDefault();try{await api(editing?`/hr/contracts/${editing.id}`:"/hr/contracts",{method:editing?"PATCH":"POST",body:JSON.stringify(f)});setOpen(false);setEditing(null);setF(blank);load()}catch(e){alert(e.message)}};return <><div className="toolbar"><div><h1>Employment Contracts</h1><p className="muted">Contract periods, probation and employment terms.</p></div><button onClick={()=>{setEditing(null);setF(blank);setOpen(true)}}>+ New Contract</button></div>{open&&<div className="panel hr-form-panel"><form className="hr-form" onSubmit={save}><div className="form-grid"><div><label>Employee *</label><select required disabled={!!editing} value={f.employee_id||""} onChange={e=>setF({...f,employee_id:e.target.value})}><option value="">Select employee</option>{emps.filter(x=>x.active).map(x=><option key={x.id} value={x.id}>{x.employee_no} — {x.full_name}</option>)}</select></div><div><label>Contract Type</label><select value={f.contract_type||"PERMANENT"} onChange={e=>setF({...f,contract_type:e.target.value})}>{["PERMANENT","FIXED_TERM","CASUAL","INTERNSHIP","CONSULTANCY"].map(x=><option key={x}>{x.replaceAll("_"," ")}</option>)}</select></div><div><label>Start Date *</label><input type="date" required value={f.start_date?.slice?.(0,10)||f.start_date||""} onChange={e=>setF({...f,start_date:e.target.value})}/></div><div><label>End Date</label><input type="date" value={f.end_date?.slice?.(0,10)||f.end_date||""} onChange={e=>setF({...f,end_date:e.target.value})}/></div><div><label>Probation End</label><input type="date" value={f.probation_end_date?.slice?.(0,10)||f.probation_end_date||""} onChange={e=>setF({...f,probation_end_date:e.target.value})}/></div><div><label>Salary</label><input type="number" min="0" step="0.01" value={f.salary||""} onChange={e=>setF({...f,salary:e.target.value})}/></div><div><label>Currency</label><select value={f.currency_code||"USD"} onChange={e=>setF({...f,currency_code:e.target.value})}><option>USD</option><option>SSP</option></select></div><div><label>Status</label><select value={f.status||"ACTIVE"} onChange={e=>setF({...f,status:e.target.value})}>{["DRAFT","ACTIVE","EXPIRED","TERMINATED","RENEWED"].map(x=><option key={x}>{x}</option>)}</select></div><div className="form-field-full"><label>Notes</label><textarea value={f.notes||""} onChange={e=>setF({...f,notes:e.target.value})}/></div></div><div className="form-actions"><button>Save Contract</button><button type="button" onClick={()=>setOpen(false)}>Cancel</button></div></form></div>}<Table cols={["employee_no","employee_name","contract_type","start_date","end_date","salary","currency_code","status"]} rows={rows} actions={r=><button onClick={()=>{setEditing(r);setF(r);setOpen(true)}}>Edit</button>}/></>;
}
