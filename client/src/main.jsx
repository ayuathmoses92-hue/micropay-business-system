...
function Quotations({user}){
  const [rows,setRows]=useState([]),[customers,setCustomers]=useState([]),[open,setOpen]=useState(false),[editing,setEditing]=useState(null),[f,setF]=useState({items:[{description:"",quantity:1,unit_price:0}]});
  const load=()=>api("/quotations").then(setRows);
  useEffect(()=>{load();api("/customers").then(setCustomers)},[]);
  const reset=()=>{setOpen(false);setEditing(null);setF({items:[{description:"",quantity:1,unit_price:0}]})};
  ...
  {open&&<div className="panel quotation-panel"><h2>{editing?`Edit ${editing.number}`:"New Quotation"}</h2><form className="quotation-form" onSubmit={save}>
    ...
    <div className="quote-items"><div className="quote-item-head"><span>Description</span><span>Qty</span><span>Unit Price (USD)</span><span></span></div>{f.items.map((x,i)=><div className="quote-item-row" key={i}><input placeholder="Description" required value={x.description} onChange={e=>updateItem(i,"description",e.target.value)}/><input type="number" min="0.001" step="0.001" placeholder="Qty" required value={x.quantity} onChange={e=>updateItem(i,"quantity",e.target.value)}/><input type="number" min="0" step="0.01" placeholder="Unit price" required value={x.unit_price} onChange={e=>updateItem(i,"unit_price",e.target.value)}/><button type="button" className="danger-button" disabled={f.items.length===1} onClick={()=>setF({...f,items:f.items.filter((_,idx)=>idx!==i)})}>Remove</button></div>)}</div>
    <button type="button" onClick={addItem}>+ Add Item</button>
    <div className="quote-summary"><div className="quote-summary-grid"><div>Sub Total (USD)</div><div>{subtotal.toFixed(2)}</div><div>Discount (USD)</div><div><input type="number" min="0" step="0.01" value={f.discount||0} onChange={e=>setF({...f,discount:e.target.value})}/></div><div>Tax (USD)</div><div><input type="number" min="0" step="0.01" value={f.tax||0} onChange={e=>setF({...f,tax:e.target.value})}/></div><div className="total-label">Total (USD)</div><div className="total-value">{total.toFixed(2)}</div></div></div>
...
function Invoices({user}){
 ...
 const edit=async r=>{try{const items=await api(`/invoices/${r.id}/items`);setEditing(r);setF({customer_id:r.customer_id,due_date:r.due_date?.slice(0,10)||"",discount:r.discount,tax:r.tax,notes:r.notes||"",items:items.map(x=>({description:x.description,quantity:x.quantity,unit_price:x.unit_price}))})}catch(err){alert(err.message)}};
...
function BudgetVsActual({user}){
  ...
  const [f,setF]=useState({budget_year:now.getFullYear(),budget_month:now.getMonth()+1,category:"Fuel",amount:"",notes:""});
...
function CashBankAccounts({user}){
  ...
  const [f,setF]=useState({account_name:"",account_type:"CASH",institution_name:"",account_reference:"",opening_balance:"",opening_balance_date:"",active:true,notes:""});
  ...
  const edit=a=>{setEditing(a);setF({account_name:a.account_name||"",account_type:a.account_type||"CASH",institution_name:a.institution_name||"",account_reference:a.account_reference||"",opening_balance:a.opening_balance||0,opening_balance_date:a.opening_balance_date||"",active:a.active!==false,notes:a.notes||""});setOpen(true)};
  ...
  return <div>
    ...
    {open&&<div className="panel"><h2>{editing?"Edit Account":"Add Financial Account"}</h2><form onSubmit={save}>
      ...
      <label>Account Type<select value={f.account_type} onChange={e=>setF({...f,account_type:e.target.value})}><option value="CASH">Cash</option><option value="BANK">Bank</option><option value="MOBILE_MONEY">Mobile Money</option></select></label>
    ...
    <section className="panel report-panel"><div className="section-title"><h2>Financial Accounts</h2><span>{accounts.length} account(s)</span></div>
      {loading?<p>Loading accounts...</p>:accounts.length?<div className="tablewrap"><table><thead><tr><th>Code</th><th>Account</th><th>Type</th><th>Institution</th><th>Current Balance</th><th>Status</th><th>Actions</th></tr></thead>
      <tbody>{accounts.map(a=><tr key={a.id}><td>{a.account_code}</td><td>{a.account_name}</td><td>{a.account_type==="MOBILE_MONEY"?"Mobile Money":a.account_type}</td><td>{a.institution_name||"—"}</td><td>{money(a.current_balance)}</td><td>{a.active?"ACTIVE":"INACTIVE"}</td>
...
function Users(){const [rows,setRows]=useState([]);const [open,setOpen]=useState(false);const [f,setF]=useState({role:"CLERK"});const load=()=>api("/auth/users").then(setRows);useEffect(load,[]);const save=async e=>{e.preventDefault();try{await api("/auth/users",{method:"POST",body:JSON.stringify(f)});setOpen(false);setF({role:"CLERK"});load()}catch(err){alert(err.message)}};const toggle=async r=>{await api(`/auth/users/${r.id}`,{method:"PATCH",body:JSON.stringify({active:!r.active})});load()};return <><div className="toolbar"><h1>Users & Roles</h1><button onClick={()=>setOpen(true)}>+ New User</button></div>{open&&<div className="panel"><form onSubmit={save}><input placeholder="Full name" required value={f.name||""} onChange={e=>setF({...f,name:e.target.value})}/><input type="email" placeholder="Email" required value={f.email||""} onChange={e=>setF({...f,email:e.target.value})}/><input type="password" placeholder="Temporary password (8+ characters)" minLength="8" required value={f.password||""} onChange={e=>setF({...f,password:e.target.value})}/><select value={f.role} onChange={e=>setF({...f,role:e.target.value})}>{Object.entries(ROLE_LABELS).map(([k,v])=><option value={k}>{v}</option>)}</select><button>Create User</button></form></div>}<Table cols={["name","email","role","active"]} rows={rows} actions={r=><button onClick={()=>toggle(r)}>{r.active?"Deactivate":"Activate"}</button>}/></>}
